...
def run_sequence(seq: Sequence, tracker: Tracker, seed, debug=False, num_gpu=8):
...
def run_dataset(dataset, trackers, seed, debug=False, threads=0, num_gpus=8):
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, seed, debug=debug)
    elif mode == 'parallel':
        param_list = [(seq, tracker_info, seed, debug, num_gpus) for seq, tracker_info in product(dataset, trackers)]
        if param_list:
            args = zip(*param_list)  # 解压为多个参数列表
        else:
            args = []
        from tqdm.contrib.concurrent import process_map
        process_map(run_sequence, *args, max_workers=threads, total=len(param_list))
