...
class NestedTensor(object):
    def __init__(self, tensors, tensors_evt, mask: Optional[Tensor]):
        self.tensors = tensors
        self.tensors_evt = tensors_evt
        self.mask = mask

    def to(self, device):
        # type: (Device) -> NestedTensor # noqa
        cast_tensor = self.tensors.to(device)
        cast_tensors_evt = self.tensors_evt.to(device)
        mask = self.mask
        if mask is not None:
            ...
            cast_mask = None
        return NestedTensor(cast_tensor, cast_tensors_evt, cast_mask)

    def decompose(self):
        return self.tensors, self.tensors_evt, self.mask

    def __repr__(self):
        return str(self.tensors) + "," + str(self.tensors_evt)
