...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    epoch = epoch - 1
    if epoch < args.TRAIN.LR_WARMUP_EPOCH:
        lr = args.TRAIN.LR * epoch / args.TRAIN.LR_WARMUP_EPOCH
    else:
        lr = args.TRAIN.MIN_LR + (args.TRAIN.LR - args.TRAIN.MIN_LR) * 0.5 * \
            (1. + math.cos(math.pi * (epoch - args.TRAIN.LR_WARMUP_EPOCH) / (args.TRAIN.EPOCH - args.TRAIN.LR_WARMUP_EPOCH)))
...
def build_dataloaders(cfg, settings):
    ...
    transform_train = tfm.Transform(tfm.ToTensorAndJitter(0.2),     # tfm.RandomHorizontalFlip_Norm(0.5),  # FE108, not normalize.
