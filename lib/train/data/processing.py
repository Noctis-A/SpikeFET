...
class STARKProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            data['template_images'], data['template_event'], data['template_anno'], data['template_masks'] = self.transform['joint'](
                image=data['template_images'], event=data['template_event'], bbox=data['template_anno'], mask=data['template_masks'])
            data['search_images'], data['search_event'], data['search_anno'], data['search_masks'] = self.transform['joint'](
                image=data['search_images'], event=data['search_event'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)
        ...
        for s in ['template', 'search']:
            ...
            # jittered_anno：扰动过后的目标框，格式为(cx, cy, w, h)，cx，cy，w，h都扰动了
            # crop_sz：扰动过后又扩大了的搜索区域的边长
            if (crop_sz < 1).any():
                data['valid'] = False
                # print("Too small box is found. Replace it with new data.")
                return data
            # Crop image region centered at jittered_anno box and get the attention mask
            crops, ev, boxes, att_mask, mask_crops, crop_coor = prutils.jittered_center_crop(data[s + '_images'],
                                                                                         data[s + '_event'],
                                                                                         jittered_anno,
                                                                                         data[s + '_anno'],
                                                                                         ...
                                                                                         masks=data[s + '_masks'])
            # Apply transforms
            data[s + '_images'], data[s + '_event'], data[s + '_anno'], data[s + '_att'], data[s + '_masks'] = self.transform[s](
                image=crops, event=ev, bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
